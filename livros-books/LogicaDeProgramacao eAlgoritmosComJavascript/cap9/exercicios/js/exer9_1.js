...
const fraseBoasVindas = document.querySelector("p");
...
const mensagemBoasVindas = () => {
	...
	if (localStorage.getItem("contadorVisitas")) {
		contador = Number(localStorage.getItem("contadorVisitas"));
	}

	if (contador == 0) {
		fraseBoasVindas.innerText = "Muito bem vindo! Esta é sua primeira visita ao nosso site.";
	} else if (contador > 0){
		fraseBoasVindas.innerText = `Que bom que voltou! Esta é sua vidita de número ${contador} ao nosso site.`;
	}
...
butao.addEventListener("click", limparLocalStorage);