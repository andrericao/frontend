...
router.get("/", async (req, res) => {
	try {
		...
		res.status(400).json({msg: error.message}); // retrona status de erro e msg
	}
});
...
router.post("/", async (req, res) => {
	...
	if(!titulo || !autor || !ano || !preco || !foto) {
		res.status(400).json({msg: "Enviar títuli, autor, ano, preco e foto do livro"});
		return;
	}
...
router.get("/dados/resumo", async (req, res) => {
	try {
		// métodos que podem ser utilizados para obter dados estáticos da tabela
...
router.get("/dados/grafico", async (req, res) => {
	try {
		...
		const total = await dbKnex("livros").select("ano")
